...
module Logic.Coalition.Evaluation.Benchmark.Runtime where
...
csvFile :: FilePath
csvFile =
  "evaluation/raw/runtime-results.csv"
...
oneCase run familyBenchmark = do
  let formula = benchmarkFormula familyBenchmark

      agents = benchmarkAgentUniverse familyBenchmark

      isexpected = expectedProvable familyBenchmark

      isvalid = isvalidformula agents formula

      agentsInCoalitions = allAgentsInCoalition formula
  ...
  if isvalid
    then do
      let cl = clWithAgents agents

          proof = isProvableZ cl formula

      startTime <- getMonotonicTimeNSec

      result <- ( try (timeout timeoutLimit (evaluate proof)) :: IO (Either SomeException (Maybe Bool)))
...
main = do
  ...
  putStrLn "===== Superadditivity ====="
  familyRunning
    [ superadditivity n
    | n <- [2 .. 21]
    ]

  putStrLn "===== Failed superadditivity ====="
  familyRunning
  ...
  putStrLn "===== Nested chain ====="
  familyRunning
    [ nestedChainCL n
    | n <- [2 .. 21]
    ]

  putStrLn "===== Runtime finished ====="