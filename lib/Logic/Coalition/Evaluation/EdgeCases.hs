...
condition2 :: Bool
condition2 =
  leftclRule fixedSetAG [] (Set.singleton (Left p)) (Left p)
    == []

--  overlapping Rejected -  CL left
condition3 :: CL
condition3 = negF (ConCL box3p (BoxCL c34 (negF p)))

-- empty coalition , m >= 1 - CL left
condition4 :: CL
condition4 =
  ImpCL
...
main :: IO ()
main = do
  putStrLn "propertyName,isValid,isExpected,actual,isAgreed"

  mapM_ printResult cases

  putStrLn
    ( "condition2"
        ++ "," ++ show (isvalidformula fixedSetAG p)
        ++ ",True"
        ++ "," ++ show condition2
        ++ "," ++ show (condition2 == True)
    )
